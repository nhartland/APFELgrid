...
interfaces to automated general-purpose Monte Carlo (MC) event
...
capabilities of the typical fitting technology. Presently a typical
global PDF fit includes thousands of hadronic data points for which
...
sums over the active partonic subprocesses in the calculation, $p_{\rm
...
Given the evolution operator defined in
Eq.~(\ref{eq:fastPDFfinal_recalled}), we may replace the
(general-scale) PDFs used in the subprocess parton density
Eq.~(\ref{eq:APPLsubproc}) with their equivalent expressions like so:
