...
\begin{equation} \label{eq:hadconv}
  \sigma_{pp\to X} =
  \sum_{i,j}^{N_F}\int dx_1\, dx_2\, f_i(x_1,Q^2)\;
...
contribution. Therefore to order $N_\alpha$ in QCD
\begin{equation}\label{eq:pertexp}
  \sigma_{pp\to X} =
  \sum_{i,j}^{N_F}\sum_{p=0}^{N_\alpha} \int dx_1\,dx_2\,
...
The central observation of tools such as {\tt APPLgrid} and {\tt
  ...
  the convolution via expansion over a set of interpolating functions in $Q^2$ and the
  two values of parton-$x$. Explicitly, one may represent the PDFs and $\alpha_S$ as
...
\end{array}
\end{equation}
in terms of interpolating functions $\mathcal{I}_\tau^{(n')}(Q^2)$,
$\mathcal{I}_\alpha^{(n_1)}(x_1)$, $\mathcal{I}_\beta^{(n_2)}(x_2)$
(typically Lagrange polynomials) of degree $n'$, $n_1$, and $n_2$.
