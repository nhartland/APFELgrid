...
%\section{The {\tt APFELgrid} package}
% **** Not sure if this sort of section would be neccesary ***
%The {\tt APFELgrid} plugin generates {\tt FK} tables of the form
%expressed in Eq.~(\ref{eq:FKTable}) from weight grids supplied by {\tt
%  APPLgrid} files. It is supplied as a set of {\tt C++} routines designed to
%  supplement the PDF evolution library {\tt APFEL} with {\tt FK} table generation
%  capabilities. The plugin has a very light interface for performing the {\tt FK} combination,
...
%*** Would now discuss e.g SIMD implementation etc in advance of looking at the results in next section ***
